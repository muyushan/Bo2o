...
public class ProductCategoryManagmentController {
    ...
    @ResponseBody
    public Map<String,Object> deleteProductCategory(Long categoryId,HttpServletRequest request){
        productDao.updateProductCategoryToNull(categoryId);
        Shop currentShop=(Shop) request.getSession().getAttribute("shop");

        ProductCategoryExecution productCategoryExecution=productCategoryService.deleteProductCategory(categoryId,currentShop.getShopId());
